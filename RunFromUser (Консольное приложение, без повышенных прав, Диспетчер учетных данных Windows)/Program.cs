/*
	Простое приложение для запуска программ от имени другого пользователя (без повышенных привелегий).
    ...
    Для обхода запуска программ с повышенными правами эта програма "RunFromUser" должна запустить программу "Run" от пользователя с административными правами,
...
namespace RunFromUser
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            StartProcessAsUser(applicationPath);    // Запуск процесса от пользователя, который сохранён в хранилище учетных данных под именем "RunFromUser"
        }
        ...
        private static bool GetUserCredentials(out string username, out string password)
        {
            using (var cred = new Credential())    // Создаём новый объект "Credential"
            {
                ...
                if (cred.Load())
                {
                    username = cred.Username;   // Присвяиваем логин
                    password = cred.Password;   // Присвяиваем пароль
